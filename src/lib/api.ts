...
export class ApiService {
  ...
  public static async getSavingGoals(): Promise<SavingGoal[]> {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.LIST, 'saving_goals');
      return [];
    }
  }
  ...
  public static async createSavingGoal(goal: Omit<SavingGoal, 'id' | 'userId'>): Promise<SavingGoal> {
    const userId = auth.currentUser?.uid;
    if (!userId) throw new Error('Not authenticated');
    const newId = `sg_${generateRandomId()}`;
    const fullGoal: any = { ...goal, userId };
    try {
      await setDoc(doc(db, 'saving_goals', newId), fullGoal);
      return { id: newId, ...fullGoal } as SavingGoal;
    } catch (err) {
      handleFirestoreError(err, OperationType.CREATE, 'saving_goals');
      throw err;
    }
  }

  public static async updateSavingGoal(id: string, goal: Partial<SavingGoal>): Promise<SavingGoal> {
    const userId = auth.currentUser?.uid;
    if (!userId) throw new Error('Not authenticated');
    try {
      const { id: _, userId: __, ...updateData } = goal as any;
      await updateDoc(doc(db, 'saving_goals', id), updateData);
      return { id, ...updateData } as SavingGoal;
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, 'saving_goals');
      throw err;
    }
  }
  ...
  public static async deleteSavingGoal(id: string): Promise<boolean> {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.DELETE, 'saving_goals');
      return false;
    }
  }

  // --- INCOMES ---
  public static async getIncomes(): Promise<Income[]> {
    const userId = auth.currentUser?.uid;
    if (!userId) return [];
    try {
  ...
  public static async createIncome(income: Omit<Income, 'id' | 'userId'>): Promise<Income> {
    ...
    const fullIncome: any = { ...income, userId };
    try {
      await setDoc(doc(db, 'incomes', newId), fullIncome);
      return { id: newId, ...fullIncome } as Income;
    } catch (err) {
  ...
  public static async updateIncome(id: string, income: Partial<Income>): Promise<Income> {
    const userId = auth.currentUser?.uid;
    if (!userId) throw new Error('Not authenticated');
    try {
      const { id: _, userId: __, ...updateData } = income as any;
      await updateDoc(doc(db, 'incomes', id), updateData);
      return { id, ...updateData } as Income;
    } catch (err) {
  ...
  public static async getRecurringExpenses(): Promise<RecurringExpense[]> {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.LIST, 'recurring_expenses');
      return [];
    }
  }
  ...
  public static async createRecurringExpense(rec: Omit<RecurringExpense, 'id' | 'userId'>): Promise<RecurringExpense> {
    ...
    const fullRec: any = { ...rec, userId };
    try {
      await setDoc(doc(db, 'recurring_expenses', newId), fullRec);
      return { id: newId, ...fullRec } as RecurringExpense;
    } catch (err) {
      handleFirestoreError(err, OperationType.CREATE, 'recurring_expenses');
      throw err;
    }
  }

  public static async updateRecurringExpense(id: string, rec: Partial<RecurringExpense>): Promise<RecurringExpense> {
    const userId = auth.currentUser?.uid;
    if (!userId) throw new Error('Not authenticated');
    try {
      const { id: _, userId: __, ...updateData } = rec as any;
      await updateDoc(doc(db, 'recurring_expenses', id), updateData);
      return { id, ...updateData } as RecurringExpense;
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, 'recurring_expenses');
      throw err;
    }
  }
  ...
  public static async deleteRecurringExpense(id: string): Promise<boolean> {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.DELETE, 'recurring_expenses');
      return false;
    }
  }
