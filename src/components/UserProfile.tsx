  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-6 py-6 sm:py-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {onBack && (
        <button
          onClick={onBack}
          className="mb-4 flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-slate-500 hover:text-slate-800 transition-colors"
        >
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-8 pt-0 relative">
          ...
          <div className="flex flex-col items-center -translate-y-12 sm:-translate-y-16 mb-[-30px] sm:mb-[-40px]">
            <div className="relative group">
              <div className="h-24 w-24 sm:h-32 sm:w-32 overflow-hidden rounded-full border-4 border-white bg-slate-100 shadow-lg relative flex items-center justify-center">
                {avatar ? (
            ...
            <h3 className="mt-3 font-display text-lg sm:text-xl font-bold text-slate-900">{name || 'Người dùng'}</h3>
            <p className="text-[11px] sm:text-xs text-slate-500 flex items-center gap-1 mt-1 font-semibold">
              <Mail className="h-3.5 w-3.5 text-slate-400" /> {user.email}
            </p>
          </div>

          <div className="space-y-4 pt-6">
            <h4 className="text-xs sm:text-sm font-bold text-slate-700 border-b border-slate-100 pb-2 uppercase tracking-wide">
              Thông tin sinh viên
            </h4>

            {successMsg && (
              <div className="flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-100 p-3 text-xs font-semibold text-emerald-700 animate-fade-in">
            ...
            {errorMsg && (
              <div className="flex items-center gap-2 rounded-xl bg-red-50 border border-red-100 p-3 text-xs font-semibold text-red-700 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative rounded-2xl border border-slate-200 focus-within:border-emerald-500 shadow-xs focus-within:ring-2 focus-within:ring-emerald-500/10 transition-all overflow-hidden bg-slate-50/20">
                  ...
                  <input
                    ...
                    className="w-full py-2.5 sm:py-3 pl-11 pr-4 text-xs sm:text-sm font-semibold text-slate-800 focus:outline-none bg-transparent"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2 bg-slate-50 p-4 rounded-2xl border border-slate-200/40">
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide">Thu nhập cố định hàng tháng</span>
                <span className="text-sm font-bold text-slate-800 mt-1 block">
                  {new Intl.NumberFormat('vi-VN').format(user.monthlyIncome)}đ
                </span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide">Mục tiêu tiết kiệm hàng tháng</span>
                <span className="text-sm font-bold text-slate-800 mt-1 block">
            ...
            <div className="flex justify-end pt-4">
              <button
                type="submit"
                disabled={isSaving}
                className="rounded-2xl bg-emerald-500 hover:bg-emerald-600 px-6 py-3 text-xs sm:text-sm font-bold text-white shadow-md shadow-emerald-100 flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 disabled:opacity-50"
              >
                {isSaving ? (
                  <>
