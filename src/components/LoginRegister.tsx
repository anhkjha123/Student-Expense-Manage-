...
export default function LoginRegister({
  ...
  const [savingGoal, setSavingGoal] = useState('500000');
  ...
  // Google popup authorized domain error diagnostic flag
  const isDomainError = errorMsg?.toLowerCase().includes('unauthorized-domain') || false;
    ...
    try {
      if (authMode === 'login') {
        ...
        const { user } = await ApiService.registerWithEmail(email, password, {
          name,
          school: school || 'Chưa cập nhật',
          monthlyIncome: Number(monthlyIncome) || 4000000,
          savingGoal: Number(savingGoal) || 500000,
        });
      ...
      console.error(err);
      // Helpful friendly message overrides
      if (err.message?.includes('auth/email-already-in-use')) {
        ...
        setErrorMsg('Tài khoản không tồn tại. Nhấn nút "Đăng ký" nếu chưa có tài khoản.');
      } else {
        setErrorMsg(err.message || 'Lỗi xử lý tài khoản. Hãy thử dùng Chế độ Ngoại tuyến.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGuestLogin = () => {
    setErrorMsg(null);
    setSuccessMsg('Đang vào Chế độ Trải nghiệm Ngoại tuyến...');
    setIsLoading(true);

    const guestId = `guest_${Date.now()}`;
    const guestUser: User = {
      id: guestId,
      email: 'guest@semtietkiem.vn',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 sm:px-6 py-10 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md space-y-6 bg-white/85 backdrop-blur-xl p-8 rounded-[2rem] border border-white/60 shadow-2xl relative z-10 transition-all duration-300">
        ...
        {/* Alerts */}
        {errorMsg && (
          <div className="rounded-2xl bg-rose-50 border border-rose-100 p-4 text-xs text-rose-800 leading-relaxed shadow-sm space-y-2.5 animate-pulse-subtle">
            ...
            {isDomainError ? (
              <div className="space-y-2">
                <p className="text-slate-600">
                  Tên miền này chưa được cấp phép truy cập Google Login trong bảng điều khiển Firebase authentication của bạn.
                </p>
                <div className="bg-white/95 p-3 rounded-xl border border-rose-200/50 text-[11px] space-y-1.5 text-slate-700 leading-normal">
                  <p className="font-bold text-slate-800 text-[11.5px]">👉 2 cách xử lý nhanh chóng nhất:</p>
                  <ol className="list-decimal pl-4.5 space-y-1 text-slate-600">
                    <li>
                      <strong>Cách 1 (Khuyên Dùng):</strong> Đăng ký tài khoản mới bằng ô <strong>"Email & Mật khẩu"</strong> bên trên, hoặc bấm <strong>"Dùng thử Offline"</strong> bên dưới. 
                      <span className="text-emerald-600 font-medium block mt-0.5">→ Sẽ không bị chặn bởi tên miền!</span>
                    </li>
                    <li className="pt-1.5">
        ...
        {successMsg && (
          <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-3 text-xs text-emerald-800 font-medium text-center shadow-sm select-none">
            {successMsg}
          </div>
        )}
          ...
          >
            {isLoading ? (
              ...
              <span className="flex items-center gap-1">
                {authMode === 'login' ? 'Tiếp tục đăng nhập' : 'Hoàn tất đăng ký'}
        ...
        <div className="space-y-2.5">
          {/* Try Offline (Highly resilient Guest Mode) */}
          <button
            type="button"
        ...
        <p className="text-[10px] text-center text-slate-400 italic">
          💡 Email & Mật khẩu và Chế độ Offline hoạt động 100% trên mọi tên miền.
        </p>
